...
// Dynamic JSON.Arr (de)serialize of the github_events payload - schema-agnostic, for
// the JSON.Obj series on the classic charts. Deserialize parses and touches a
// GitHub event metadata projection so the benchmark does not measure a touch-nothing parse.
...
function sumGithubProjection(root: JSON.Arr): f64 {
  ...
  for (let i = 0, n = root.length; i < n; i++) {
    ...
    s +=
      sumStringField(event, "type") +
      sumStringField(event, "created_at") +
      sumStringField(event, "id");
    const actor = objField(event, "actor");
    if (actor !== null)
      s += sumStringField(actor, "login") + sumNumberField(actor, "id");
    const repo = objField(event, "repo");
    if (repo !== null)
      s += sumStringField(repo, "name") + sumNumberField(repo, "id");
    ...
    if (payload !== null) {
      ...
      s +=
        sumNumberField(payload, "size") +
        sumNumberField(payload, "distinct_size");
      const issue = objField(payload, "issue");
      if (issue !== null)
        s += sumStringField(issue, "title") + sumNumberField(issue, "id");
      const comment = objField(payload, "comment");
      if (comment !== null)
        s += sumStringField(comment, "body") + sumNumberField(comment, "id");
    }
  }
  return s;
}
...
bench(
  "Deserialize github_events (JSON.Obj, pretty)",
  () => {
    blackbox(sumGithubProjection(JSON.parse<JSON.Arr>(prettyJson)));
  },
  20000,
  utf8ByteLength(prettyJson),
);
dumpToFile("github_events-obj-pretty", "deserialize");

bench(
  "Deserialize github_events (JSON.Obj, min)",
  () => {
    blackbox(sumGithubProjection(JSON.parse<JSON.Arr>(minJson)));
  },
  20000,
  utf8ByteLength(minJson),
);
dumpToFile("github_events-obj-min", "deserialize");

bench(
  "Serialize github_events (JSON.Obj, min)",
  () => {
...
dumpToFile("github_events-obj-min", "serialize");
