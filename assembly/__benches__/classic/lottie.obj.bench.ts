...
// Dynamic JSON.Obj (de)serialize of the lottie payload - schema-agnostic, for
// the JSON.Obj series on the classic charts. Deserialize parses and touches a
// Lottie metadata and layer projection so the benchmark does not measure a touch-nothing parse.
...
function sumLayerProjection(layer: JSON.Obj): f64 {
  let s =
    sumStringField(layer, "nm") +
    sumNumberField(layer, "ty") +
    sumNumberField(layer, "ip") +
    sumNumberField(layer, "op");
...
function sumLottieProjection(root: JSON.Obj): f64 {
  let s =
    sumStringField(root, "v") +
    sumNumberField(root, "fr") +
    sumNumberField(root, "w") +
    sumNumberField(root, "h") +
    sumNumberField(root, "op");
  const layers = arrField(root, "layers");
  if (layers !== null) {
    for (let i = 0, n = layers.length; i < n; i++)
      s += sumLayerProjection(layers.at(i).get<JSON.Obj>());
  }
  ...
  if (assets !== null) {
    for (let i = 0, n = assets.length; i < n; i++) {
      ...
      if (assetLayers !== null) {
        for (let j = 0, m = assetLayers.length; j < m; j++)
          s += sumLayerProjection(assetLayers.at(j).get<JSON.Obj>());
      }
    }
  }
  return s;
...
bench(
  "Deserialize lottie (JSON.Obj, pretty)",
  () => {
    blackbox(sumLottieProjection(JSON.parse<JSON.Obj>(prettyJson)));
  },
  3000,
  utf8ByteLength(prettyJson),
);
dumpToFile("lottie-obj-pretty", "deserialize");

bench(
  "Deserialize lottie (JSON.Obj, min)",
  () => {
    blackbox(sumLottieProjection(JSON.parse<JSON.Obj>(minJson)));
  },
  3000,
  utf8ByteLength(minJson),
);
dumpToFile("lottie-obj-min", "deserialize");

bench(
  "Serialize lottie (JSON.Obj, min)",
  () => {
...
dumpToFile("lottie-obj-min", "serialize");
