...
// Dynamic JSON.Obj (de)serialize of the otfcc payload - schema-agnostic, for
// the JSON.Obj series on the classic charts. Deserialize parses and touches a
// OTFCC top-level table projection so the benchmark does not measure a touch-nothing parse.
const prettyJson = readFile("./assembly/__benches__/payloads/otfcc.min.json");
...
bench(
  "Deserialize otfcc (JSON.Obj, pretty)",
  () => {
    blackbox(sumOtfccProjection(JSON.parse<JSON.Obj>(prettyJson)));
  },
  40,
  utf8ByteLength(prettyJson),
);
dumpToFile("otfcc-obj-pretty", "deserialize");

bench(
  "Deserialize otfcc (JSON.Obj, min)",
  () => {
    blackbox(sumOtfccProjection(JSON.parse<JSON.Obj>(minJson)));
  },
  40,
  utf8ByteLength(minJson),
);
dumpToFile("otfcc-obj-min", "deserialize");

bench(
  "Serialize otfcc (JSON.Obj, min)",
  () => {
...
dumpToFile("otfcc-obj-min", "serialize");
