...
// @ts-ignore: decorator valid here
@inline function skipWs(ptr: usize, end: usize): usize {
...
//
// The scalar scanners step one u16 (2 bytes) at a time. On a large document
// most of those bytes are *inside* string values (e.g. long URLs) — content we
// only want to skip past to reach the next structural character. These helpers
// load 8 u16 lanes per step (`v128`), compare against the structural set with
// `i16x8.eq`, fold to an 8-bit mask, and `ctz` to the first hit — ~8× fewer
// loads through that filler. Same technique as `deserialize/simd/string.ts`.
//
// Requires `--enable simd`. A scalar tail handles the final < 8 lanes; the
// `ptr + 16 <= end` guard keeps every wide load inside [start, end), which is
// bounded by the source string's allocation.

// @ts-ignore: decorator valid here
@lazy const S_QUOTE: v128 = i16x8.splat(0x22); // "
// @ts-ignore: decorator valid here
@lazy const S_BSLASH: v128 = i16x8.splat(0x5c); // \
// @ts-ignore: decorator valid here
@lazy const S_BRACE_L: v128 = i16x8.splat(0x7b); // {
// @ts-ignore: decorator valid here
@lazy const S_BRACE_R: v128 = i16x8.splat(0x7d); // }
// @ts-ignore: decorator valid here
@lazy const S_BRACK_L: v128 = i16x8.splat(0x5b); // [
// @ts-ignore: decorator valid here
@lazy const S_BRACK_R: v128 = i16x8.splat(0x5d); // ]

/**
 * Scan a string body (from its opening quote) to the closing quote, SIMD-wide.
 * Returns the close-quote position for an UNESCAPED string, or `0` if a `\`
 * appears first (the signal to fall back to the escape-correct scanner). The
 * first-of-{`"`,`\`} a block reports answers exactly "is there an escape before
 * the close?" — `ctz` picks the lowest-address hit of either.
 */
// @ts-ignore: decorator valid here
@inline function scanPlainString(start: usize, end: usize): usize {
  let p = start + 2; // past the opening quote
  while (p + 16 <= end) {
    const block = load<v128>(p);
    const mask = i16x8.bitmask(
      v128.or(i16x8.eq(block, S_QUOTE), i16x8.eq(block, S_BSLASH)),
    );
    if (mask) {
...
/**
 * Scalar twin of `scanPlainString`, kept only for the SIMD-vs-scalar microbench
 * (`bench.ts`). Same contract: close-quote position, or `0` on a leading escape.
 */
// @ts-ignore: decorator valid here
@inline export function scanPlainStringScalar(start: usize, end: usize): usize {
  let p = start + 2;
  while (p < end) {
    const c = load<u16>(p);
    if (c == QUOTE) return p;
...
/** Bench shim: scan a string value's content to its close, SIMD vs scalar. */
// @ts-ignore: decorator valid here
@inline export function strScanSIMD(loc: Loc): i32 {
  const s = skipWs(locStart(loc), locEnd(loc));
  return <i32>((scanPlainString(s, locEnd(loc)) - s) >> 1);
}
// @ts-ignore: decorator valid here
@inline export function strScanScalar(loc: Loc): i32 {
  const s = skipWs(locStart(loc), locEnd(loc));
  return <i32>((scanPlainStringScalar(s, locEnd(loc)) - s) >> 1);
}

/**
 * Position of the next structural char in {`{` `}` `[` `]` `"`} at or after
 * `ptr`, SIMD-wide; `end` if none. Used to skip filler inside a container while
...
@inline function nextStructural(ptr: usize, end: usize): usize {
  while (ptr + 16 <= end) {
    ...
    const mask = i16x8.bitmask(
      v128.or(
        v128.or(
          i16x8.eq(block, S_QUOTE),
          v128.or(i16x8.eq(block, S_BRACE_L), i16x8.eq(block, S_BRACE_R)),
        ),
        v128.or(i16x8.eq(block, S_BRACK_L), i16x8.eq(block, S_BRACK_R)),
      ),
    );
 ...
 */
// @ts-ignore: decorator valid here
@inline function scanValueEnd(ptr: usize, end: usize): usize {
  const c = load<u16>(ptr);
 ...
 */
// @ts-ignore: decorator valid here
@inline function scanKeyEnd(ptr: usize, end: usize): usize {
...
// @ts-ignore: decorator valid here
@inline function memEq(a: usize, b: usize, bytes: usize): bool {
  let i: usize = 0;
  for (; i + 8 <= bytes; i += 8)
...
// ---------------------------------------------------------------------------
// Demo — exercises the lazy surface end-to-end.
// ---------------------------------------------------------------------------
export function demo(): void {
  const doc = Document.from(
    '{"name":"ada","age":36,"tags":["math","engine"],"meta":{"alive":true}}',
  );
  const root = doc.root;

  // each access scans only the path walked — the rest of the doc is untouched
  trace("name", 1, <f64>root.get("name")!.asString().length); // -> 3 ("ada")
  trace("age", 1, <f64>root.get("age")!.asI32()); // -> 36
  trace("tags.length", 1, <f64>root.get("tags")!.length); // -> 2
  trace("tags[1] kind", 1, <f64>root.get("tags")!.at(1)!.kind); // -> Kind.String
  trace(
    "meta.alive",
    1,
    <f64>(root.get("meta")!.get("alive")!.asBool() ? 1 : 0),
  );
}
