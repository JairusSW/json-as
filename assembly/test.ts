...
/**
 * Helper: serialize then deserialize a string
 */
...
describe("deserializeString_SIMD", () => {
    ...
    it("handles mixed content", () => {
        const str = `Line1\nLine2\tTabbed\\Backslash"Quote\u0003End`;
        expect(roundTrip(str)).toBe(str);
    });
    ...
    it("handles very long strings", () => {
        let str = "";
        for (let i = 0; i < 5000; i++) str += "abc123\\\"";
        expect(roundTrip(str)).toBe(str);
    });

    it("handles strings ending with escape sequences", () => {
        expect(roundTrip("EndsWithBackslash\\")).toBe("EndsWithBackslash\\");
    ...
    it("handles boundary cases near SIMD lanes", () => {
        let str = "";
        for (let i = 0; i < 16; i++) str += String.fromCharCode(65 + i); // A-P
        str += "\\\""; // escape at lane boundary
        expect(roundTrip(str)).toBe(str);

        str = "1234567890123456\\u0041"; // exactly 16 + escape
        expect(roundTrip(str)).toBe(str);
    });
