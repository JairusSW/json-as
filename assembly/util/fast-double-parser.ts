// Port of fast_double_parser (Apache 2.0) from https://github.com/lemire/fast_double_parser
// Original authors: Daniel Lemire and contributors.
// This AssemblyScript port parses ASCII numeric JSON tokens from UTF-16 input.

const FASTFLOAT_SMALLEST_POWER: i32 = -325;
const FASTFLOAT_LARGEST_POWER: i32 = 308;

const POWER_OF_TEN = memory.data<f64>([
  1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11,
  1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18, 1e19, 1e20, 1e21, 1e22,
]);

const MANTISSA_64 = memory.data<u64>([
...
const MANTISSA_128 = memory.data<u64>([
...
let _mul128Low: u64 = 0;

@inline function fullMulHigh(x: u64, y: u64): u64 {
  const a = <u32>(x >>> 32);
  const b = <u32>x;
  const c = <u32>(y >>> 32);
  const d = <u32>y;
  ...
  const ad = <u64>a * d;
  const bd = <u64>b * d;
  const mid = (bd >>> 32) + (<u32>ad) + (<u32>bc);
  _mul128Low = (mid << 32) | (<u64>(<u32>bd));
  return ac + (ad >>> 32) + (bc >>> 32) + (mid >>> 32);
}

@inline function leadingZeroes64(x: u64): i32 {
  const hi = <u32>(x >>> 32);
  if (hi != 0) return clz(hi);
  return 32 + clz(<u32>x);
}

let _fdpSuccess: bool = false;
let _fdpEnd: usize = 0;

@inline function computeFloat64(power: i32, i: u64, negative: bool): f64 {
  _fdpSuccess = false;
  ...
  if (power >= -22 && power <= 22 && i <= 9007199254740991) {
    let d = <f64>i;
    if (power < 0) {
      d = d / load<f64>(POWER_OF_TEN + ((-power) << 3));
    } else {
      d = d * load<f64>(POWER_OF_TEN + (power << 3));
    }
    if (negative) d = -d;
  ...
  if (power < FASTFLOAT_SMALLEST_POWER || power > FASTFLOAT_LARGEST_POWER) return 0.0;

  const idx = (power - FASTFLOAT_SMALLEST_POWER) << 3;
  const factorMantissa = load<u64>(MANTISSA_64 + idx);

  let exponent: i64 = (((152170 + 65536) * power) >> 16) + 1024 + 63;
  let lz = leadingZeroes64(i);
  let value = i << lz;

  const upper = fullMulHigh(value, factorMantissa);
  let lower = _mul128Low;
  let upperHi = upper;

  if ((upperHi & 0x1FF) == 0x1FF && (lower + value < lower)) {
    const factorMantissaLow = load<u64>(MANTISSA_128 + idx);
    const upper2 = fullMulHigh(value, factorMantissaLow);
    const productLow = _mul128Low;
    let productMiddle1 = lower;
    let productMiddle2 = upper2;
    let productHigh = upperHi;
    let productMiddle = productMiddle1 + productMiddle2;
    if (productMiddle < productMiddle1) productHigh++;
    if ((productMiddle + 1 == 0) && ((productHigh & 0x1FF) == 0x1FF) && (productLow + value < productLow)) {
      return 0.0;
    }
    upperHi = productHigh;
    lower = productMiddle;
  ...
  const upperbit = upperHi >>> 63;
  let mantissa = upperHi >>> (upperbit + 9);
  lz += i32(1 ^ upperbit);

  if ((lower == 0) && ((upperHi & 0x1FF) == 0) && ((mantissa & 3) == 1)) {
    return 0.0;
  }
  ...
  mantissa &= ~(1 << 52);
  const realExponent = exponent - lz;
  if (realExponent < 1 || realExponent > 2046) return 0.0;

  let bits = <u64>mantissa | (<u64>realExponent << 52);
  if (negative) bits |= 0x8000_0000_0000_0000;
  const d = f64.reinterpret_i64(<i64>bits);
  _fdpSuccess = true;
  return d;
}

@inline function isDigit(code: u16): bool {
  return <u32>code - 48 <= 9;
}

export function fastDoubleParse(srcStart: usize, srcEnd: usize, outPtr: usize): usize {
  _fdpEnd = 0;
  _fdpSuccess = false;
  ...
  if (code == 48) { // '0'
    p += 2;
    if (p < srcEnd && isDigit(load<u16>(p))) return 0;
    i = 0;
  } else {
    if (!isDigit(code)) return 0;
    i = <u64>(code - 48);
    p += 2;
    while (p < srcEnd && isDigit(load<u16>(p))) {
      i = i * 10 + <u64>(load<u16>(p) - 48);
      p += 2;
    }
  }

  let exponent: i64 = 0;
  let firstAfterPeriod: usize = 0;
  ...
  if (p < srcEnd && load<u16>(p) == 46) { // '.'
    ...
    exponent = <i64>((firstAfterPeriod - p) >> 1);
  }

  let digitCount = <i32>(((p - startDigits) >> 1) - 1);

  if (p < srcEnd) {
    const expCode = load<u16>(p);
    if (expCode == 101 || expCode == 69) { // 'e' or 'E'
      p += 2;
      if (p >= srcEnd) return 0;
      let negExp = false;
      let ec = load<u16>(p);
      ...
      if (!isDigit(ec)) return 0;
      let expNumber: i64 = <i64>(ec - 48);
      p += 2;
      if (p < srcEnd && isDigit(load<u16>(p))) {
        expNumber = expNumber * 10 + <i64>(load<u16>(p) - 48);
        p += 2;
      }
      if (p < srcEnd && isDigit(load<u16>(p))) {
        expNumber = expNumber * 10 + <i64>(load<u16>(p) - 48);
  ...
  if (digitCount >= 19) {
    let start = startDigits;
    while (start < p) {
      const c = load<u16>(start);
      if (c != 48 && c != 46) break;
      start += 2;
    }
    digitCount -= <i32>((start - startDigits) >> 1);
    if (digitCount >= 19) return 0;
  }

  if (exponent < FASTFLOAT_SMALLEST_POWER || exponent > FASTFLOAT_LARGEST_POWER) return 0;

  const d = computeFloat64(<i32>exponent, i, negative);
  if (!_fdpSuccess) return 0;
...
export function fastDoubleParseEnd(): usize {
  return _fdpEnd;
}
