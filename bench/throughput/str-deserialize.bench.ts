...
bench("Deserialize Small String (1kb)", () => blackbox(JSON.parse(strSmallStr)), 3_000_000, strSmallStr.length << 1);
dumpToFile("small-str", "deserialize");

bench("Deserialize Medium String (500kb)", () => blackbox(JSON.parse(strMediumStr)), 8_500, strMediumStr.length << 1);
dumpToFile("medium-str", "deserialize");

bench("Deserialize Large String (1000kb)", () => blackbox(JSON.parse(strLargeStr)), 3_000, strLargeStr.length << 1);
