...
bench("Serialize Small String (1kb)", () => blackbox(JSON.stringify(strSmall)), 3_000_000, strSmallStr.length);
dumpToFile("small-str", "serialize");

bench("Serialize Medium String (500kb)", () => blackbox(JSON.stringify(strMedium)), 8_500, strMediumStr.length);
dumpToFile("medium-str", "serialize");

bench("Serialize Large String (1000kb)", () => blackbox(JSON.stringify(strLarge)), 3_000, strLargeStr.length);
