...
async function main(argv) {
    ...
    for (const warning of schema.warnings)
        console.warn(`[json-as-schema] ${warning.path}: ${warning.message}`);
    if (options.out)
...
function parseArgs(argv) {
    const options = { files: [], jsonl: false };
    ...
    if (options.files.length === 0)
        throw new Error("Expected at least one JSON input file");
    if (!options.rootName)
        options.rootName = guessRootName(options.files[0]);
    return options;
}
...
async function loadSamples(files, jsonl) {
    const samples = [];
