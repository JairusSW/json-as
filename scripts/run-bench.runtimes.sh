...
# not time anything externally. Two builds cover the runtimes' differing host
...
#
#   * WASI build  -> Wago / wasmtime / wavm / wazero (payload read via WASI;
#                    results to stdout as __AS_BENCH_JSON__ lines)
#   * env  build  -> v8 / bun                  (env-ABI host bench/runners/
#                    runtimes-env.mjs supplies readFile/performance.now/...)
#
# Wago is driven through a small Go runner that pins the engine and supplies the
# same WASI Preview 1 surface as the standalone WASI runtimes.
set -euo pipefail
...
mkdir -p "$WASM_DIR" "$LOG_DIR"

# Common asc flags: NAIVE with a shared feature subset, so all runtimes execute
# equivalent code.
COMMON_FLAGS=(-O3 --noAssert --uncheckedBehavior always --runtime incremental
  --disable nontrapping-f2i --disable bulk-memory)
...
# --- build the Wago host --------------------------------------------------
WAGO_RUNNER="$WASM_DIR/wago-run"
build_wago_runner() {
  has go || return 1
  echo "==> building Wago runner"
  go build -C bench/runners/wago -o "$ROOT_DIR/$WAGO_RUNNER" .
}
...
WAGO_AVAILABLE=0
if build_wago_runner; then
  WAGO_AVAILABLE=1
else
  echo "    (Wago skipped - runner build failed or Go toolchain not found)"
fi
...
for name in "${PAYLOADS[@]}"; do
  ...
  # --- WASI build: Wago / wasmtime / wavm / wazero ------------------------
  if ((WAGO_AVAILABLE)) || has wasmtime || has wavm || has wazero; then
    ...
    if ((WAGO_AVAILABLE)); then
      "$WAGO_RUNNER" "$wasi_wasm" 2>/dev/null | capture wago
    fi
